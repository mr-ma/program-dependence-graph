...
class SVFGCSV : public llvm::ModulePass {
public:
  static char ID;
  SVFGCSV() : llvm::ModulePass(ID) {}
  ...
  bool runOnModule(llvm::Module &M) override {
    ...
    std::vector<std::pair<std::size_t, std::size_t>> blockSCCRelationIds,
        blockSVFRelationIds, blockCFGRelationIds;
	...
	for(BasicBlock *succBB: successors(&B)){
	  ...
	  hasSuccBlocks = true;
        ...
        std::vector<std::pair<std::size_t, std::size_t>> BBOutBBs;
	...
	bool hasSVF = false;
        for (auto &I : B) {
          ...
          auto instOutBBs = getOutEdgeBB(svfg, &I, &F, blockId);
	  ...
	  if(BBLabel.compare("none")!=0){
        ...
        blockFeatures.push_back(
            std::make_tuple(blockId, featureVector, BBLabel, blockContent));
    ...
    for (auto &[bid, features, label, content] : blockFeatures) {
      if (std::find(soloBlocks.begin(), soloBlocks.end(), bid) ==
          soloBlocks.end()) {
        dumpBBFeatures(blockfile, bid, features, label, content);
      } else {
    ...
    for (auto &[sBid, dBid] : blockCFGRelationIds) {
      if (std::find(soloBlocks.begin(), soloBlocks.end(), sBid) ==
        ...
        if (std::find(soloBlocks.begin(), soloBlocks.end(), dBid) ==
            soloBlocks.end())
          fprintf(blockrelfile, "%zu;%zu;%s;\n", sBid, dBid, "cfg");
    }

    dbgs() << "SVG rel:" << blockSVFRelationIds.size() << "\n";
    for (auto &[sBid, dBid] : blockSVFRelationIds) {
      if (std::find(soloBlocks.begin(), soloBlocks.end(), sBid) ==
          soloBlocks.end())
        if (std::find(soloBlocks.begin(), soloBlocks.end(), dBid) ==
            soloBlocks.end())
    ...
    llvm::dbgs() << "Soloblocks:" << soloBlocks.size() << "\n";
    return false;
  }
  ...
  void dumpBBFeatures(FILE *blockfile, const size_t BBId,
                      const std::vector<int> featureVector, std::string BBLabel,
                      std::string BBContent) {
    fprintf(blockfile, "%zu;", BBId);
    for (int feature : featureVector) {
      fprintf(blockfile, "%i;", feature);
    }
    fprintf(blockfile, "%s;%s;\n\r", BBContent.c_str(), BBLabel.c_str());
  }
  std::vector<std::pair<std::size_t, std::size_t>>
  getOutEdgeBB(SVFG *svfg, llvm::Value *I, llvm::Function *F,
               std::size_t blockId) {
    auto *pag = svfg->getPAG();
    std::vector<std::pair<std::size_t, std::size_t>> outBBEdges;
    if (!pag->hasValueNode(I)) {
      return outBBEdges;
    }
    auto nodeId = pag->getValueNode(I);
    auto *pagNode = pag->getPAGNode(nodeId);
    if (!hasOutgoingEdges(pagNode)) {
      return outBBEdges;
    }
    ...
    for (auto edge_it = svfgNode->OutEdgeBegin();
      ...
      auto *node = (*edge_it)->getDstNode();
      // Check if the edge is to or from another BB
      if (auto *stmtNode = llvm::dyn_cast<StmtSVFGNode>(svfgNode)) {
        // auto *stmtsvgnode =const_cast<StmtSVFGNode *>(stmtNode);
        auto *nodeBB = stmtNode->getInst()->getParent();
        if (nodeBB != nullptr && svfgNode->getBB() != nullptr) {
          if (nodeBB != svfgNode->getBB()) {
            outBBEdges.push_back(std::make_pair(
                blockId, getUniqueBlockName(nodeBB, nodeBB->getParent())));
          }
        }
      }
      if (node->getBB() != nullptr && svfgNode->getBB() != node->getBB()) {
        outBBEdges.push_back(std::make_pair(
            blockId,
            getUniqueBlockName(node->getBB(), node->getBB()->getParent())));
      }
    }
    return outBBEdges;
  }
...
char SVFGCSV::ID = 0;
static llvm::RegisterPass<SVFGCSV>
    X("svfg-csv", "Traverse SVFG graph and print information in CSV");
